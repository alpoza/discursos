...
pcorpus = corpus(discursos_raw %>% split(.$docid) %>% map(~ paste0(.x, collapse = " ")) %>% map_chr(~ paste0(.x,collapse= " ")))
options(width = 200)
kwic(pcorpus, "iraq")
...
terrorismo_keywords = c("atentad*","terror*","eta","gal","yihad*")
...
corrplot(relacion_docs, method = "pie", type="lower", order = "hclust")
